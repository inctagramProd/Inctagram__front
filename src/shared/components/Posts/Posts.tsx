...
const Posts = ({name, img, postDescreption, likes, comments}: PostsProps) => {
    ...
    return (
        <div className="flex flex-col">
            ...
            <div className="w-full h-[30px] flex mt-[10px] mb-[10px] items-center justify-between">
          ...
          <span className={'text-dark-100 cursor-pointer italic'} onClick={openDescription}>
            ...
            {likes?.length > 0 ? (
                <div className="flex flex-row w-full h-[40px] items-center">
                  ...
                  <div className={'ml-[14px]'}>  {`   ${likes?.length}`} </div> <span className="inline-block font-bold pl-[5px]">{pageText.like}</span>
                </div>
            ) : null}
