...
export const Select = ({
    ...
    const toggleVisibilityDropdown = (): void => {
    ...
    return (
        <div className="max-w-sm">
            ...
            <div onClick={toggleVisibilityDropdown} className={`${isVisibleDropdown ? 'border-light-100' : 'border-dark-100' } w-full cursor-pointer text-light-100 w-1 border px-4 py-1 rounded-sm flex justify-between hover:text-light-900`}>
                {options[0].title}
                <div>
            ...
            <div className={`transition-all ${isVisibleDropdown ? 'block' : 'hidden'} border bg-dark-500 py-1 rounded-b-sm`}>
            {
                options.map(item => {
                    return <div className="py-1 px-4 hover:bg-dark-300 hover:text-primary-500">{item.title}</div>
                })
            }
            </div>
        </div>
