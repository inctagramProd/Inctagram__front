...
export const Modal = ({ isOpen, onOpenChange, email }: Props) => {
  ...
  return (
    <Dialog.Root open={isOpen} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 bg-dark-900 bg-opacity-50" />
        <Dialog.Content className="fixed bg-dark-300 rounded top-1/3 left-1/2 transform -translate-x-1/2 -translate-y-1/2 border border-dark-100 max-w-[378px]">
