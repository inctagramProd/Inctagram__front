...
Ваши обязательства. В обмен на наше обязательство предоставлять Сервис вы должны взять на себя следующие обязательства перед нами. Кто может использовать Inсtagram. Мы хотим, чтобы наш Сервис был максимально открытым и доступным для всех, но вместе с тем безопасным, защищенным и соответствующим требованиям законодательства. Поэтому, чтобы стать частью сообщества Inсtagram, вы должны соблюдать некоторые ограничения.
    ...
    `,
        ...
        authErrors: {
            ...
            emailField: {
                ...
                email: 'Электронная почта должна соответствовать формату example@example.com.',
            },
            terms:
                'Пожалуйста, ознакомьтесь и примите Правила сервиса и Политику конфиденциальности',
            password: {
                nonEmpty: 'Введите  пароль',
                regex: 'Пароль должен содержать a-z, A-Z, ! " # $ % & \' ( ) * + , - . / : ; < = > ? @ [ \\ ] ^ _` { | } ~',
                min: 'Минимальное количество символов 6',
                max: 'Максимальное количество символов 20',
            },
            recaptcha: {
                notARobot: 'Я не робот',
                verifyPlease: 'Пожалуйста, подтвердите, что вы не робот',
        ...
        profileSetting: {
            ...
            profileSettingsErrors: {
                ...
                lastNameField: {
                    ...
                    regex:
                        'Фамилия может содержать символы A-z; А-я; -; апостроф; пробел',
                    min: 'Минимальное количество символов 1',
                    max: 'Максимальное количество символов 50',
                },
                avatarError: {
...
addComment:'Добавить комментарий',like:' "Нравится"'},
        addNewPost: {
            ...
            saveDraft: 'Сохранить',
            close: 'Закрыть',
            successCreate: 'Пост успешно опубликован',
            imageError: {
                size: 'Размер изображения не должен превышать 0.5 MB',
                format: 'Формат загружаемого изображения должен быть PNG или JPEG',
                file: 'Ошибка при обработке файла',
                upload: 'Вы пытаетесь добавить слишком много изображений к этому посту. Пожалуйста, ограничьте количество изображений до 10 штук для одного поста'
            }
        },
        descriptionError: {
            error: 'Максимальное количество знаков 500',
        },
        editPost: {
